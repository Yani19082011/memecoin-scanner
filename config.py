...
# Свален от 200000 (18.09, по изричен избор на потребителя: първо "търси
# такива още в началото си примерно на 30k mc и да стигат до 90k mc", после
# уточнено на "50к до 110к mc примерно" - реален случай, който доведе до
# първата промяна: FKhooZdA...pump, score 79, market cap $104,981, momentum
# +98.3% - алъртът излезе С ПРЕДУПРЕЖДЕНИЕ "вече силно изпомпана",
# потребителят все пак влезе и загуби $4. Целта е да хващаме растежа РАНО
# (~30k-110k), не след него - виж EXTENDED_MAX_MARKET_CAP_USD по-долу за
# по-нататъшното "продължено следене" на монети, които останат чисти отвъд
# този таван (потребителят изрично поиска и двете - "и early alert, и
# продължено следене").
MAX_MARKET_CAP_USD = float(os.getenv("MAX_MARKET_CAP_USD", "110000"))

# --- "Разширена зона" за истински трайни тенденции (не бърз pump-and-dump) ---
# По изричен избор на потребителя (18.09): "ако е свързано с нещо тренд ново
# ... long runner примерно от 30k mc до 500k mc". Не можем надеждно да
# познаем автоматично дали дадена монета е "нов тренд" (нямаме безплатен
# API за това) - затова вместо да гадаем по име/тема, пускаме монета над
# MAX_MARKET_CAP_USD (110k) да продължи да се следи САМО ако е АБСОЛЮТНО
# чиста по всички други, обективни признаци (виж score_token - "разширена
# зона" секцията): 0 RugCheck риск флага (не просто под 'danger'), дълбоко
# заключена ликвидност, нормално (не wash-traded) съотношение обем/ликвидност
# И пропорционално по-дълбока минимална ликвидност от нормалното. Ако дори
# едно от тези не е изпълнено - отхвърля се на обичайния таван от 110k, както
# преди. Над 500k - твърд таван, никога не продължаваме да следим.
EXTENDED_MAX_MARKET_CAP_USD = float(os.getenv("EXTENDED_MAX_MARKET_CAP_USD", "500000"))
# По-висок под на ликвидност за "разширената зона" - по-голям market cap без
# пропорционално по-дълбока ликвидност е точно профилът на wash trading /
# лесна за манипулация монета, не на здрав траен растеж.
EXTENDED_ZONE_MIN_LIQUIDITY_USD = float(os.getenv("EXTENDED_ZONE_MIN_LIQUIDITY_USD", "40000"))
